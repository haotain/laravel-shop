      ...
      <tbody class="product_list">
        @foreach($cartItems as $item)
          <tr data-id="{{ $item->productSku->id }}">
            ...
            <td>
              <input type="text" class="form-control form-control-sm amount" @if(!$item->productSku->product->on_sale) desabled @endif name="amount" value="{{ $item->amount }}">
            </td>
            <td>
...
<script>
  $(document).ready(function() {
    ...
    $('.btn-create-order').click(async function() {
      ...
      try {
        response = await axios.post("{{ route('orders.store') }}", req)
        swal('订单提交成功', '', 'success')
          .then(() => {
